...
static void imuCalculateEstimatedAttitude(timeUs_t currentTimeUs)
{
    ...
    if (sensors(SENSOR_MAG) && compassIsHealthy()) {
        ...
        // In case of a fixed-wing aircraft (or under certain conditions with a miniquad) we can use GPS course over ground to correct heading
        rawYawError = DECIDEGREES_TO_RADIANS(attitude.values.yaw - gpsSol.groundCourse);
	//XXX to do: define miniquad conditions to accept heading correcting via cog
        useYaw = true;
    }
#endif
